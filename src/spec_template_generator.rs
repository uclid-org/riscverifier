...
use crate::readers::dwarfreader::{DwarfCtx, DwarfVar, DwarfTypeDefn};
use crate::utils;
...
impl SpecTemplateGenerator {
    ...
    pub fn fun_templates(func_names: &HashSet<String>, dwarf_ctx: &DwarfCtx) -> Result<String, utils::Error> {
    ...
    fn type_to_string(typ: &DwarfTypeDefn) -> String {
        match typ {
            ...
            DwarfTypeDefn::Array { in_typ, out_typ, bytes } => format!("bv{}", bytes*utils::BYTE_SIZE),
            DwarfTypeDefn::Struct { id, fields, bytes } => format!("bv{}", bytes*utils::BYTE_SIZE),
            DwarfTypeDefn::Pointer { value_typ, bytes } => format!("bv{}", bytes*utils::BYTE_SIZE),
        }
    }
}
