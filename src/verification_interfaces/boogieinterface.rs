...
#[derive(Debug)]
pub struct BoogieInterface;

impl BoogieInterface {
    ...
    fn prelude() -> String {
        fs::read_to_string(utils::BOOGIE_PRELUDE_PATH).expect("Unable to read prelude.")
    }
    ...
    fn gen_array_defn(typ_defn: &DwarfTypeDefn) -> Vec<String> {
        ...
        match &typ_defn {
            DwarfTypeDefn::Primitive { bytes } => {
                ...
                if *bytes > 0 {
                    defns.push(format!(
                        "function {{:inline}} {}(base: xlen_t, index: xlen_t): xlen_t {{ bv64add(base, {}) }}",
        ...
        match typ {
            DwarfTypeDefn::Struct {
                ...
                for (field_name, field) in fields {
                    ...
                    defns.push(format!(
                        "function {{:inline}} {}(ptr: xlen_t): xlen_t {{ bv64add(ptr, to_xlen_t({}bv64)) }}",
...
impl IRInterface for BoogieInterface {
    ...
    fn bv_app_to_string(bvop: &BVOp, e1: Option<String>, e2: Option<String>) -> String {
        match bvop {
            BVOp::Add => format!("bv64add({}, {})", e1.unwrap(), e2.unwrap()),
            BVOp::Sub => format!("bv64sub({}, {})", e1.unwrap(), e2.unwrap()),
            BVOp::Mul => format!("bv64mul({}, {})", e1.unwrap(), e2.unwrap()),
            BVOp::And => format!("bv64and({}, {})", e1.unwrap(), e2.unwrap()),
            BVOp::Or => format!("bv64or({}, {})", e1.unwrap(), e2.unwrap()),
            BVOp::Xor => format!("bv64xor({}, {})", e1.unwrap(), e2.unwrap()),
    ...
    fn func_call_to_string(func_call: &FuncCall) -> String {
        ...
        format!(
            "call {}{}({});",
            if lhs.len() != 0 { format!("{} := ", lhs) } else { String::from("") },
    ...
    fn model_to_string(
        ...
        model: &Model,
        dwarf_ctx: &DwarfCtx,
        _ignored_funcs: &HashSet<&str>,
        _verify_funcs: &Vec<&str>,
    ) -> String {
        let xlen_defn = utils::indent_text(
            format!(
                "type xlen_t = bv{};\nfunction {{:inline}} to_xlen_t(x: bv64): xlen_t {{ x[{}:0] }}",
                xlen,
                xlen,
            ),
            4,
